...
- take two buttons which prints odd and even and in first state increment one variable till 1 billion then print odd/even 
    - kya hoga?
        - entire application will slow down or the browser will hang (depends on the system)

- useMemo():
    - ek sada pyaaj saraa container ko sada deta hai, useMemo() removes that rot pyaaj 
...
```
- Your cbf passed to the useMemo() hook, if has any expensive/heavy calculation/computation, so it could affect the overall performance of the application but as it is used with the useMemo() hook, it won't get called on each component re-render. It will get called only when there are some changes in dependencies and it will return the memoized value